...
class APIRecommender(object):
  ...
  def add_query(self, query):
    ...
    self.query_id_in_mashups_df = mashups_df.shape[0] - 1

    apis_df = self.apis_df.append({ 'description_words': self.text_filter_function(query) }, ignore_index = True)
    self.query_id_in_apis_df = apis_df.shape[0] - 1

    tf_idf = TfidfVectorizer(analyzer = str.split, max_df = 0.85, max_features = 10000)
  ...
  def recommend_apis_from_apis(self, api_id, k = 15):
    ...
    for i in range(self.query_id_in_apis_df):
